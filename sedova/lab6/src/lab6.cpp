...
int main() {
	...
	std::cout << '\n';
	Result << '\n';
	std::cout << "Номер интервала:\t" << "Левая граница:\t" << "Количество чисел, попавших в интервал:\n";
	Result << "Номер интервала:\t" << "Левая граница:\t" << "Количество чисел, попавших в интервал:\n";
	for (int i = 0; i < NInt; ++i) {
		std::cout << "    " << i + 1 << "\t        \t" << LGrInt[i] << "\t\t" << result[i] << '\n';
