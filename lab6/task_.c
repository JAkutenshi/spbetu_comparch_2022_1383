...
extern void function(int *res_arr, int *nums_arr, int k_inter, int count_N,
                     int *left_arr, int *max_res);
...
int main() {
  ...
  int *max_res = calloc(k_inter, sizeof(int));
  for (int i = 0; i < k_inter; i++) {
    max_res[i] = INT_MIN;
  }

  function(res, nums, k_inter, countN, arr, max_res);
